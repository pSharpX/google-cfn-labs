...
@Getter
@Setter
@PropertiesPrefix(value = "logbook")
public final class LogbookProperties {
    ...
    public static class ExtractorProperty {
        ...
        public AttributeExtractor toExtractor(@Nonnull final ObjectMapper objectMapper) {
            return switch (type) {
                case "JwtFirstMatchingClaimExtractor" -> JwtFirstMatchingClaimExtractor.builder()
                        .objectMapper(objectMapper)
                        .claimNames(claimNames)
                        .claimKey(claimKey)
                        .build();
                case "JwtAllMatchingClaimsExtractor" -> JwtAllMatchingClaimsExtractor.builder()
                        .objectMapper(objectMapper)
                        .claimNames(claimNames)
                        .build();
                default -> throw new IllegalArgumentException("Unknown AttributeExtractor type: " + type);
            };
        }
    }
